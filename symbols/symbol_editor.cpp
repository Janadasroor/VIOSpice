...
#include "editor/symbol_pin_table_panel.h"
#include "editor/symbol_wizard_panel.h"
#include "editor/symbol_library_browser_panel.h"
...
SymbolEditor::~SymbolEditor() {
    if (m_canvas) m_canvas->removeOverlayItems();
    // Clear undo stack while m_canvas->scene() is still alive, so command destructors
...
void SymbolEditor::applyTheme() {
    ...
    if (m_libraryBrowser) {
        m_libraryBrowser->applyTheme();
    }

    if (m_pinTablePanel) {
        m_pinTablePanel->applyTheme();
    }
    ...
    if (m_canvas) {
        QPalette vp = m_canvas->palette();
        vp.setColor(QPalette::Highlight, QColor("#90caf9"));
        m_canvas->setPalette(vp);
...
void SymbolEditor::applySymbolDefinition(const SymbolDefinition& def) {
    ...
    m_canvas->m_drawnItems.reserve(effective.size());
    for (int i = 0; i < effective.size(); ++i) {
        ...
        QGraphicsItem* visual = m_canvas->buildVisual(effective[i], i);
        if (visual) {
            if (isInherited) {
            ...
            m_canvas->scene()->addItem(visual);
            m_canvas->m_drawnItems.append(visual);
    ...
    m_canvas->updateOverlayLabels();
    updateCodePreview();
    if (m_pinTablePanel) {
        m_pinTablePanel->updatePinTable();
    }
    updateSubcktMappingTable();
    m_canvas->updateGuideAnchors();
...
void SymbolEditor::setupUI() {
    ...
    // ── Main Canvas ─────────────────────────────────────────────────────────
    m_canvas = new SymbolCanvas(this, this);
    ...
    m_libraryBrowser = new SymbolLibraryBrowserPanel(this, libDock);
    libDock->setWidget(m_libraryBrowser);
    addDockWidget(Qt::LeftDockWidgetArea, libDock);

    auto* wizDock = new QDockWidget("IC Wizard", this);
    wizDock->setObjectName("WizardDock");
    m_wizardPanel = new SymbolWizardPanel(m_canvas, this);
    m_wizardPanel->setProjectKey(m_projectKey);
    wizDock->setWidget(m_wizardPanel);
    addDockWidget(Qt::LeftDockWidgetArea, wizDock);

    connect(m_wizardPanel, &SymbolWizardPanel::symbolGenerated,
            this, &SymbolEditor::onWizardSymbolGenerated);
    connect(m_wizardPanel, &SymbolWizardPanel::importSpiceSubcircuitRequested,
            this, &SymbolEditor::onImportSpiceSubcircuit);
    connect(m_wizardPanel, &SymbolWizardPanel::importKicadSymbolRequested,
            this, &SymbolEditor::onImportKicadSymbol);
    connect(m_wizardPanel, &SymbolWizardPanel::importLtspiceSymbolRequested,
            this, &SymbolEditor::onImportLtspiceSymbol);
    ...
    m_pinTablePanel = new SymbolPinTablePanel(this, pinDock);
    pinDock->setWidget(m_pinTablePanel);
    ...
    connect(m_srcList, &QListWidget::itemClicked, this, [this](QListWidgetItem* item) {
        ...
        if (idx >= 0 && idx < m_canvas->m_drawnItems.size()) {
            m_canvas->scene()->clearSelection();
            m_canvas->m_drawnItems[idx]->setSelected(true);
            m_canvas->centerOn(m_canvas->m_drawnItems[idx]);
        }
    });
...
void SymbolEditor::connectViewSignals() {
    if (!m_canvas) return;
    connect(m_canvas, &SymbolCanvas::contextMenuRequested, this, &SymbolEditor::onCanvasContextMenu);
    connect(m_canvas, &SymbolCanvas::rotateCWRequested,  this, &SymbolEditor::onRotateCW);
    connect(m_canvas, &SymbolCanvas::rotateCCWRequested, this, &SymbolEditor::onRotateCCW);
    connect(m_canvas, &SymbolCanvas::flipHRequested,     this, &SymbolEditor::onFlipH);
    connect(m_canvas, &SymbolCanvas::flipVRequested,     this, &SymbolEditor::onFlipV);
    connect(m_canvas, &SymbolCanvas::coordinatesChanged, this, &SymbolEditor::updateCoordinates);
    connect(m_canvas, &SymbolCanvas::itemErased,         this, &SymbolEditor::onItemErased);
}
...
void SymbolEditor::createMenuBar() {
    ...
    connect(selectAllAct, &QAction::triggered, this, [this]() {
        if (!m_canvas->scene()) return;
        for (auto* it : m_canvas->scene()->items()) it->setSelected(true);
    });
    ...
    connect(snapCursorCrosshairAct, &QAction::toggled, this, [this](bool on) {
        if (m_canvas) m_canvas->setSnapCursorCrosshairEnabled(on);
        if (statusBar()) {
...
void SymbolEditor::createStatusBar() {
    ...
    connect(unitCombo, &QComboBox::currentTextChanged, this, [this](const QString& unit) {
        m_canvas->setProperty("currentUnit", unit);
        updateCoordinates(m_canvas->mapToScene(m_canvas->mapFromGlobal(QCursor::pos())));
    });
...
void SymbolEditor::updateCoordinates(QPointF pos) {
    if (m_coordLabel) {
        QString unit = m_canvas->property("currentUnit").toString();
...
void SymbolEditor::onWizardSymbolGenerated(const SymbolDefinition& def) {
    if (!m_canvas->m_drawnItems.isEmpty()) {
    ...
    else if (newDef.name().isEmpty()) newDef.setName("New_IC");

    if (!prefix.isEmpty()) newDef.setReferencePrefix(prefix);
    else if (newDef.referencePrefix().isEmpty()) newDef.setReferencePrefix("U");

    if (!category.isEmpty()) newDef.setCategory(category);

    m_undoStack->push(new UpdateSymbolCommand(this, oldDef, newDef, "Wizard Generate"));
}

void SymbolEditor::onWizardSaveTemplate() {
    if (m_wizardPanel) {
        m_wizardPanel->saveTemplate(symbolDefinition());
    }
}
...
void SymbolEditor::onClear() {
    if (m_canvas->m_drawnItems.isEmpty()) return;
...
void SymbolEditor::onDelete() {
    ...
    for (QGraphicsItem* vis : m_canvas->m_drawnItems) {
        if (!vis || !vis->isSelected()) continue;
        int idx = m_canvas->primitiveIndex(vis);
...
void SymbolEditor::onToolSelected() {
    ...
    if (!action) return;
    Tool tool = static_cast<Tool>(action->data().toInt());
    ...
    // Clear pen state inside canvas when switching tools
    if (tool != Pen) {
        m_canvas->clearPenState();
    }
    
    m_canvas->setCurrentTool(static_cast<int>(tool));
    m_canvas->setDragMode(tool == Select ? QGraphicsView::RubberBandDrag
                                         : QGraphicsView::NoDrag);
    m_canvas->m_polyPoints.clear();
    if (m_canvas->m_previewItem) {
        m_canvas->scene()->removeItem(m_canvas->m_previewItem);
...
bool SymbolEditor::saveSymbolToLibrary() {
    ...
    if (selection.startsWith("[Dir] ")) {
        ...
        Q_EMIT symbolSaved(m_symbol);
        if (m_undoStack) m_undoStack->setClean();
        if (m_libraryBrowser) m_libraryBrowser->populateLibraryTree();
...
void SymbolEditor::onRefreshLibraries() {
    if (m_libraryBrowser) {
        m_libraryBrowser->onRefreshLibraries();
    }
}
...
void SymbolEditor::onRotateCW() {
    ...
    if (!centerFound) {
        if (selected.size() == 1) {
            center = m_canvas->snapToGrid(selected.first()->sceneBoundingRect().center());
        } else {
            QRectF bbox;
            for (auto* item : selected) bbox = bbox.united(item->sceneBoundingRect());
            center = m_canvas->snapToGrid(bbox.center());
        }
    }
...
void SymbolEditor::onDistributeH() {
    ...
    for (int i = 1; i < selected.size() - 1; ++i) {
        int idx = m_canvas->primitiveIndex(selected[i]);
        if (idx != -1) {
...
void SymbolEditor::onMatchSpacing() {
    ...
    double pitch = QInputDialog::getDouble(this, "Match Spacing", "Enter spacing (mm):", 
                                          m_canvas->gridSize(), 0.1, 100.0, 2, &ok);
    if (!ok) return;
...
void SymbolEditor::onSnapToGrid() {
    ...
    for (auto* item : selected) {
        ...
        if (prim.type == SymbolPrimitive::Pin) {
            ...
            QPointF snapped = m_canvas->snapToGrid(p);
            if (p != snapped) {
...
void SymbolEditor::onImportImage() {
    ...
    QGraphicsItem* visual = m_canvas->buildVisual(prim, m_symbol.primitives().size());
    if (visual) {
...
void SymbolEditor::onAiSymbolGenerated(const QString& json) {
    ...
    m_canvas->fitInView(m_canvas->scene()->itemsBoundingRect().adjusted(-20,-20,20,20),
                      Qt::KeepAspectRatio);
...
void SymbolEditor::onZoomIn()  { if (m_canvas) m_canvas->zoomByFactor(1.25); }
void SymbolEditor::onZoomOut() { if (m_canvas) m_canvas->zoomByFactor(1.0 / 1.25); }
void SymbolEditor::onZoomFit() {
    if (!m_canvas || !m_canvas->scene()) return;
    QRectF bounds = m_canvas->scene()->itemsBoundingRect();
    if (bounds.isNull()) return;
    bounds.adjust(-20, -20, 20, 20);
    m_canvas->fitInView(bounds, Qt::KeepAspectRatio);
}
void SymbolEditor::onZoomSelection() {
    if (!m_canvas || !m_canvas->scene()) return;
    QRectF bounds;
    for (auto* item : m_canvas->scene()->selectedItems()) bounds = bounds.united(item->sceneBoundingRect());
    if (!bounds.isNull()) m_canvas->fitInView(bounds.adjusted(-10,-10,10,10), Qt::KeepAspectRatio);
}
...
void SymbolEditor::onSelectionChanged() {
    if (m_propsTabWidget && m_canvas && !m_canvas->scene()->selectedItems().isEmpty()) {
    ...
    if (m_canvas) m_canvas->updateResizeHandles();
}
...
void SymbolEditor::onPropertyChanged(const QString& name, const QVariant& value) {
    ...
    if (selected.size() == 1) {
        ...
        int index = m_canvas->primitiveIndex(item);
...
void SymbolEditor::keyPressEvent(QKeyEvent* event) {
    ...
    const int toolInt = m_canvas->currentTool();
    Tool tool = static_cast<Tool>(toolInt);

    if (event->key() == Qt::Key_Escape) {
        // Finalize pen path if active
        if (tool == Pen && m_canvas->m_penPoints.size() >= 2) {
            m_canvas->finalizePenPath();
        }
        // Switch to select tool
        m_canvas->setCurrentTool(0);
        m_canvas->setDragMode(QGraphicsView::RubberBandDrag);
        m_canvas->m_polyPoints.clear();
        m_canvas->clearPenState();
        m_canvas->m_penFinalizing = false;  // Reset guard flag after all cleanup
        if (m_selectAction) m_selectAction->setChecked(true);
        event->accept();
        return;
    } else if (event->key() == Qt::Key_Delete ||
               (event->key() == Qt::Key_Backspace && tool != Pen)) {
        onDelete();
        event->accept();
        return;
    } else if (event->key() == Qt::Key_Return && tool == Pen) {
        // Enter key: finalize pen path
        m_canvas->finalizePenPath();
        event->accept();
        return;
    } else if (event->key() == Qt::Key_Backspace && tool == Pen && !m_canvas->m_penPoints.isEmpty()) {
        // Backspace: remove last point
        m_canvas->m_penPoints.removeLast();
        m_canvas->updatePenPreview();
        event->accept();
        return;
    } else if (event->key() == Qt::Key_R && tool == Pin) {
        ...
        statusBar()->showMessage("Pin orientation: " + m_canvas->m_previewOrientation, 1200);
        event->accept();
        return;
    } else if (event->key() == Qt::Key_H && tool == Pin
...
void SymbolEditor::onCanvasContextMenu(const QPoint& pos) {
    QGraphicsItem* item = m_canvas->itemAt(pos);
    QMenu menu(this);
    ...
    if (item) {
        ...
        addMenu->addAction(getThemeIcon(":/icons/tool_pin.svg"), "Pin", [this, pos]() {
            if (auto* a = m_toolActions.value("Pin")) {
                a->trigger();
                if (m_canvas) m_canvas->updatePinPreview(m_canvas->mapToScene(pos));
            }
        });
        addMenu->addAction(getThemeIcon(":/icons/tool_line.svg"), "Line", [this]() { if (auto* a = m_toolActions.value("Line")) a->trigger(); });
        addMenu->addAction(getThemeIcon(":/icons/tool_rect.svg"), "Rectangle", [this]() { if (auto* a = m_toolActions.value("Rect")) a->trigger(); });
        addMenu->addAction(getThemeIcon(":/icons/tool_circle.svg"), "Circle", [this]() { if (auto* a = m_toolActions.value("Circle")) a->trigger(); });
        addMenu->addAction(getThemeIcon(":/icons/tool_arc.svg"), "Arc", [this]() { if (auto* a = m_toolActions.value("Arc")) a->trigger(); });
        addMenu->addAction(getThemeIcon(":/icons/tool_polygon.svg"), "Polygon", [this]() { if (auto* a = m_toolActions.value("Polygon")) a->trigger(); });
        addMenu->addAction(getThemeIcon(":/icons/tool_bezier.svg"), "Bezier Curve", [this]() { if (auto* a = m_toolActions.value("Bezier")) a->trigger(); });
        addMenu->addAction(getThemeIcon(":/icons/tool_pen.svg"), "Pen Tool", [this]() { if (auto* a = m_toolActions.value("Pen")) a->trigger(); });
        addMenu->addAction(getThemeIcon(":/icons/tool_text.svg"), "Text", [this, pos]() {
            if (auto* a = m_toolActions.value("Text")) {
                a->trigger();
                if (m_canvas) {
                    QPointF scenePos = m_canvas->snapToGrid(m_canvas->mapToScene(pos));
                    Q_EMIT m_canvas->pointClicked(scenePos);
                }
            }
        });
        addMenu->addSeparator();
        addMenu->addAction(getThemeIcon(":/icons/tool_anchor.svg"), "Set Anchor Point", [this]() { if (auto* a = m_toolActions.value("Anchor")) a->trigger(); });
        ...
        snapAct->setChecked(m_canvas ? m_canvas->snapToGridEnabled() : true);
        connect(snapAct, &QAction::toggled, this, [this](bool on) {
            if (m_canvas) m_canvas->setSnapToGrid(on);
        });
    }
