...
DesignReportData DesignReportGenerator::collectData(QGraphicsScene* scene)
{
    ...
    data.boardName = QStringLiteral("PCB Design");
    data.boardVersion = QStringLiteral("1.0");
    data.generatedAt = QDateTime::currentDateTime().toString(QStringLiteral("yyyy-MM-dd HH:mm:ss"));
    ...
    for (const auto& sl : stackup.stack) {
        ...
        li.name = sl.name;
        li.type = sl.type;
        li.side = (sl.layerId == 0) ? QStringLiteral("Top")
                  : (sl.layerId == 1) ? QStringLiteral("Bottom")
    ...
    for (auto* item : scene->items()) {
        if (auto* comp = dynamic_cast<ComponentItem*>(item)) {
            ...
            QString layerStr = (layerId == 1) ? QStringLiteral("Bottom") : QStringLiteral("Top");
            layerCounts[layerStr]++;

            const QString value = comp->value().isEmpty() ? QStringLiteral("-") : comp->value();
            valueCounts[value]++;
        }
    }
    ...
    data.componentsByValue = valueCounts;
    ...
    data.drcInfos = std::count_if(drc.violations().begin(),
                                  drc.violations().end(),
                                  [](const DRCViolation& v) {
                                      return v.severity() == DRCViolation::Info;
                                  });
    ...
    for (const auto& v : drc.violations()) {
        ...
        info.location = QStringLiteral("(%1, %2)")
    ...
    for (auto* item : scene->items()) {
        if (auto* comp = dynamic_cast<ComponentItem*>(item)) {
            QString key = comp->value() + QStringLiteral("|") + comp->componentType();
...
QString DesignReportGenerator::generateHTML(const DesignReportData& data,
                                            const ReportOptions& options)
{
    ...
    out << "<title>PCB Design Report - " << esc(data.boardName) << "</title>\n";
    out << "<style>\n";
    ...
    out << ".warning { color: #b8860b; font-weight: bold; }\n";
    ...
    if (!options.projectName.isEmpty()) {
        out << "<tr><td><b>Project</b></td><td>" << esc(options.projectName) << "</td></tr>\n";
    }

    if (!options.companyName.isEmpty()) {
        out << "<tr><td><b>Company</b></td><td>" << esc(options.companyName) << "</td></tr>\n";
    }

    out << "<tr><td><b>Board</b></td><td>" << esc(data.boardName) << "</td></tr>\n";

    if (!data.boardVersion.isEmpty()) {
        out << "<tr><td><b>Version</b></td><td>" << esc(data.boardVersion) << "</td></tr>\n";
    }

    if (!data.author.isEmpty()) {
        out << "<tr><td><b>Author</b></td><td>" << esc(data.author) << "</td></tr>\n";
    }

    out << "<tr><td><b>Generated</b></td><td>" << esc(data.generatedAt) << "</td></tr>\n";
    out << "<tr><td><b>Board Size</b></td><td>"
        << QString::number(data.boardWidth, 'f', 2)
        << " x "
        << QString::number(data.boardHeight, 'f', 2)
        << " mm</td></tr>\n";
    ...
    if (options.includeStatistics) {
        ...
        out << "<tr><td><b>Components</b></td><td class=\"stat\">"
            << data.totalComponents << "</td></tr>\n";

        out << "<tr><td><b>Nets</b></td><td class=\"stat\">"
            << data.totalNets << "</td></tr>\n";

        out << "<tr><td><b>Trace Segments</b></td><td class=\"stat\">"
            << data.totalTraceSegments << "</td></tr>\n";

        out << "<tr><td><b>Total Trace Length</b></td><td class=\"stat\">"
            << QString::number(data.totalTraceLength, 'f', 2) << " mm</td></tr>\n";

        out << "<tr><td><b>Vias</b></td><td class=\"stat\">"
            << data.totalVias << "</td></tr>\n";

        out << "<tr><td><b>Copper Pours</b></td><td class=\"stat\">"
            << data.copperPours << "</td></tr>\n";

        out << "<tr><td><b>Board Thickness</b></td><td>"
            << QString::number(data.boardThickness, 'f', 2) << " mm</td></tr>\n";

        out << "<tr><td><b>Surface Finish</b></td><td>"
            << (data.surfaceFinish.isEmpty() ? QStringLiteral("N/A") : esc(data.surfaceFinish))
            << "</td></tr>\n";
        ...
        for (auto it = data.componentsByLayer.begin(); it != data.componentsByLayer.end(); ++it) {
            out << "<tr><td>" << esc(it.key()) << "</td><td>" << it.value() << "</td></tr>\n";
        }
        out << "</table>\n";
    }
    ...
    if (options.includeLayers && !data.layers.isEmpty()) {
        ...
        for (const auto& li : data.layers) {
            const QString thickness = QString::number(li.thickness, 'f', 3);
            const QString copper = li.copperWeightOz > 0
                                       ? QString::number(li.copperWeightOz, 'f', 1)
                                       : QStringLiteral("-");

            out << "<tr><td>" << esc(li.name)
                << "</td><td>" << esc(li.type)
                << "</td><td>" << esc(li.side)
                << "</td><td>" << thickness
                << "</td><td>" << (li.material.isEmpty() ? QStringLiteral("-") : esc(li.material))
                << "</td><td>" << copper
                << "</td></tr>\n";
        }
    ...
    if (options.includeDRC) {
        ...
        out << "<tr><td><b>Errors</b></td><td class=\""
            << (data.drcErrors > 0 ? "error" : "ok") << "\">"
            << data.drcErrors << "</td></tr>\n";

        out << "<tr><td><b>Warnings</b></td><td class=\""
            << (data.drcWarnings > 0 ? "warning" : "ok") << "\">"
            << data.drcWarnings << "</td></tr>\n";

        out << "<tr><td><b>Info</b></td><td>" << data.drcInfos << "</td></tr>\n";

        QString status = (data.drcErrors == 0)
                             ? QStringLiteral("<span class=\"ok\">PASS</span>")
                             : QStringLiteral("<span class=\"error\">FAIL</span>");
        ...
        if (!data.drcViolations.isEmpty()) {
            ...
            const int maxViolations = 1000;

            for (int i = 0; i < data.drcViolations.size() && i < maxViolations; ++i) {
                const auto& v = data.drcViolations.at(i);

                QString sevClass;
                if (v.severity == QStringLiteral("Error")) {
                    sevClass = QStringLiteral("error");
                } else if (v.severity == QStringLiteral("Warning")) {
                    sevClass = QStringLiteral("warning");
                }

                out << "<tr><td class=\"" << sevClass << "\">" << esc(v.severity)
                    << "</td><td>" << esc(v.type)
                    << "</td><td>" << esc(v.message)
                    << "</td><td>" << esc(v.location)
                    << "</td></tr>\n";
            }

            if (data.drcViolations.size() > maxViolations) {
                out << "<tr><td colspan=\"4\">"
                    << (data.drcViolations.size() - maxViolations)
                    << " more violations not shown.</td></tr>\n";
            }
    ...
    if (options.includeNets) {
        ...
        out << "<tr><td><b>Unrouted Nets</b></td><td class=\""
            << (data.unroutedNets > 0 ? "warning" : "ok") << "\">"
            << data.unroutedNets << "</td></tr>\n";

        out << "<tr><td><b>Unrouted Airwires</b></td><td>" << data.totalAirwires << "</td></tr>\n";

        out << "<tr><td><b>Total Trace Length</b></td><td>"
            << QString::number(data.totalTraceLength, 'f', 2) << " mm</td></tr>\n";
    ...
    if (options.includeComponents) {
        if (!data.componentsByType.isEmpty()) {
            out << "<h2>5. Components by Footprint Type</h2>\n";
            out << "<table>\n<tr><th>Footprint Type</th><th>Count</th></tr>\n";

            QStringList typeKeys = data.componentsByType.keys();
            std::sort(typeKeys.begin(), typeKeys.end(), [&data](const QString& a, const QString& b) {
                return data.componentsByType[a] > data.componentsByType[b];
            });

            for (const QString& key : typeKeys) {
                out << "<tr><td>" << esc(key) << "</td><td>"
                    << data.componentsByType[key] << "</td></tr>\n";
            }

            out << "</table>\n";
        }

        if (!data.componentsByValue.isEmpty()) {
            out << "<h3>Components by Value</h3>\n";
            out << "<table>\n<tr><th>Value</th><th>Count</th></tr>\n";

            QStringList valueKeys = data.componentsByValue.keys();
            std::sort(valueKeys.begin(), valueKeys.end(), [&data](const QString& a, const QString& b) {
                return data.componentsByValue[a] > data.componentsByValue[b];
            });

            for (const QString& key : valueKeys) {
                out << "<tr><td>" << esc(key) << "</td><td>"
                    << data.componentsByValue[key] << "</td></tr>\n";
            }
    ...
    if (options.includeNetClasses && !data.netClasses.isEmpty()) {
        ...
        out << "<table>\n<tr><th>Class</th><th>Trace Width (mm)</th><th>Clearance (mm)</th><th>Via Diameter (mm)</th><th>Via Drill (mm)</th><th>Nets</th></tr>\n";

        for (const auto& nc : data.netClasses) {
            out << "<tr><td>" << esc(nc.name)
                << "</td><td>" << QString::number(nc.traceWidth, 'f', 2)
    ...
    if (options.includeBOM && !data.bomSummary.isEmpty()) {
        ...
        QList<DesignReportData::BOMSummary> sortedBom = data.bomSummary;
        std::sort(sortedBom.begin(),
                  sortedBom.end(),
                  [](const DesignReportData::BOMSummary& a,
                     const DesignReportData::BOMSummary& b) {
                      return a.quantity > b.quantity;
                  });

        for (const auto& bs : sortedBom) {
            QStringList escapedRefs;
            escapedRefs.reserve(bs.references.size());

            for (const QString& ref : bs.references) {
                escapedRefs << esc(ref);
            }

            out << "<tr><td>" << (bs.value.isEmpty() ? QStringLiteral("-") : esc(bs.value))
                << "</td><td>" << esc(bs.footprint)
                << "</td><td>" << escapedRefs.join(QStringLiteral(", "))
                << "</td><td>" << bs.quantity
                << "</td></tr>\n";
        }
    ...
    out << "<p>Generated by <b>VioraEDA</b> on " << esc(data.generatedAt) << "</p>\n";

    if (!options.companyName.isEmpty()) {
        out << "<p>" << esc(options.companyName) << " &mdash; Confidential</p>\n";
    }
...
bool DesignReportGenerator::generateReport(QGraphicsScene* scene,
                                           const QString& filePath,
                                           const ReportOptions& options,
                                           QString* errorMessage)
{
    QString outPath = filePath.trimmed();

    if (outPath.isEmpty()) {
        if (errorMessage) {
            *errorMessage = QStringLiteral("Output file path is empty.");
        }
        return false;
    }
    ...
    if (fi.suffix().isEmpty()) {
        outPath += (options.format == HTML) ? QStringLiteral(".html") : QStringLiteral(".pdf");
        fi.setFile(outPath);
    }

    if (!QDir().mkpath(fi.absolutePath())) {
        if (errorMessage) {
            *errorMessage = QStringLiteral("Cannot create output directory for: %1").arg(outPath);
        }
        return false;
    }
    ...
    if (data.boardName.isEmpty()) {
        data.boardName = options.projectName.isEmpty()
                             ? QStringLiteral("PCB Design")
                             : options.projectName;
    }
    ...
    if (options.format == HTML) {
        ...
        if (!file.open(QIODevice::WriteOnly | QIODevice::Text)) {
            if (errorMessage) {
                *errorMessage = QStringLiteral("Cannot open output file: %1").arg(outPath);
            }
            return false;
        }
    ...
    writer.setTitle(QStringLiteral("PCB Design Report - %1").arg(data.boardName));
    ...
    if (!outFi.exists() || outFi.size() == 0) {
        if (errorMessage) {
            *errorMessage = QStringLiteral("Failed to create PDF file: %1").arg(outPath);
        }
        return false;
    }
