...
bool GerberExporter::exportLayer(QGraphicsScene* scene, int layerId, const QString& filePath, const GerberExportSettings& settings) {
    ...
    const int decimals = 6; // Standard Gerber X2 4.6 decimal format

    // 1. KiCad 8 Standard Gerber X2 Header
    out << "%FSLAX46Y46*%\n"; // Standard 4.6 decimal format (sub-micron precision)
    out << "%MOMM*%\n";       // Metric units (mm)
    out << "%LPD*%\n";        // Layer Polarity Dark

    // Generation Software & Date X2 Attributes
    out << "%TF.GenerationSoftware,VioSpice,VioraEDA,1.0.0*%\n";
    out << QString("%%TF.CreationDate,%1*%%\n").arg(QDateTime::currentDateTimeUtc().toString(Qt::ISODate));

    // Layer Function X2 Attribute (%TF.FileFunction...)
    if (layerId == PCBLayerManager::TopCopper) {
        out << "%TF.FileFunction,Copper,L1,Top*%\n";
    } else if (layerId == PCBLayerManager::BottomCopper) {
        out << "%TF.FileFunction,Copper,L2,Bot*%\n";
    } else if (layerId == PCBLayerManager::TopSoldermask) {
        out << "%TF.FileFunction,Soldermask,Top*%\n";
    } else if (layerId == PCBLayerManager::BottomSoldermask) {
        out << "%TF.FileFunction,Soldermask,Bot*%\n";
    } else if (layerId == PCBLayerManager::TopSilkscreen) {
        out << "%TF.FileFunction,Legend,Top*%\n";
    } else if (layerId == PCBLayerManager::BottomSilkscreen) {
        out << "%TF.FileFunction,Legend,Bot*%\n";
    } else if (layerId == PCBLayerManager::TopPaste) {
        out << "%TF.FileFunction,Paste,Top*%\n";
    } else if (layerId == PCBLayerManager::BottomPaste) {
        out << "%TF.FileFunction,Paste,Bot*%\n";
    } else if (layerId == PCBLayerManager::EdgeCuts) {
        out << "%TF.FileFunction,Profile*%\n";
    }
...
bool GerberExporter::exportGerberJobFile(QGraphicsScene* scene,
                                          const QString& outputDirectory,
    ...
    const QString cleanName = projectName.isEmpty() ? "VioSpice_Project" : projectName;
    const QString jobFilePath = dir.filePath(cleanName + ".gbrjob");
    ...
    for (QGraphicsItem* item : scene->items()) {
        if (PCBItem* pcb = dynamic_cast<PCBItem*>(item)) {
            if (pcb->layer() == PCBLayerManager::EdgeCuts) {
                bounds = pcb->sceneTransform().map(pcb->shape()).boundingRect();
                break;
            }
        }
    }
    ...
    // Header
    QJsonObject header;
    header["Comment"] = "Gerber X2 Job File generated by VioSpice EDA";
    header["CreationDate"] = QDateTime::currentDateTimeUtc().toString(Qt::ISODate);
    QJsonObject spec;
    spec["Vendor"] = "VioSpice";
    spec["Application"] = "VioraEDA";
    spec["Version"] = "1.0.0";
    header["GenerationSoftware"] = spec;
    root["Header"] = header;

    // GeneralSpecs
    QJsonObject genSpecs;
    QJsonObject projId;
    projId["Name"] = cleanName;
    projId["GUID"] = cleanName + "_VioSpice_GUID";
    projId["Revision"] = "1.0";
    genSpecs["ProjectId"] = projId;
    
    QJsonObject size;
    size["X"] = std::round(bounds.width() * 1000.0) / 1000.0;
    size["Y"] = std::round(bounds.height() * 1000.0) / 1000.0;
    genSpecs["Size"] = size;
    genSpecs["LayerNumber"] = 2;
    genSpecs["BoardThickness"] = 1.6;
    genSpecs["Finish"] = "None";
    root["GeneralSpecs"] = genSpecs;

    // FilesAttributes
    QJsonArray filesAttrs;
    auto addFileAttr = [&](const QString& path, const QString& role) {
        QJsonObject f;
        f["Path"] = QFileInfo(path).fileName();
        f["FileFunction"] = role;
        f["FilePolarity"] = "Positive";
        filesAttrs.append(f);
    };

    addFileAttr(cleanName + "-F_Cu.gbr", "Copper,L1,Top");
    addFileAttr(cleanName + "-B_Cu.gbr", "Copper,L2,Bot");
    addFileAttr(cleanName + "-F_Paste.gbr", "Paste,Top");
    addFileAttr(cleanName + "-B_Paste.gbr", "Paste,Bot");
    addFileAttr(cleanName + "-F_Silkscreen.gbr", "Legend,Top");
    addFileAttr(cleanName + "-B_Silkscreen.gbr", "Legend,Bot");
    addFileAttr(cleanName + "-F_Mask.gbr", "Soldermask,Top");
    addFileAttr(cleanName + "-B_Mask.gbr", "Soldermask,Bot");
    addFileAttr(cleanName + "-Edge_Cuts.gbr", "Profile");
    root["FilesAttributes"] = filesAttrs;

    // MaterialStackup
    QJsonArray stackup;
    auto addStackup = [&](const QString& type, const QString& name, double thickness, const QString& mat = "") {
        QJsonObject s;
        s["Type"] = type;
        s["Name"] = name;
        if (thickness > 0) s["Thickness"] = thickness;
        if (!mat.isEmpty()) s["Material"] = mat;
        stackup.append(s);
    };

    addStackup("Legend", "Top Silkscreen", 0.015);
    addStackup("SolderMask", "Top Solder Mask", 0.010);
    addStackup("Copper", "Top Copper", 0.035);
    addStackup("Dielectric", "FR4 Core Substrate", 1.510, "FR4");
    addStackup("Copper", "Bottom Copper", 0.035);
    addStackup("SolderMask", "Bottom Solder Mask", 0.010);
    addStackup("Legend", "Bottom Silkscreen", 0.015);
    root["MaterialStackup"] = stackup;

    QFile file(jobFilePath);
