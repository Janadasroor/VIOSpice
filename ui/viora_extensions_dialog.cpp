...
#include "viora_extensions_dialog.h"
...
VioraExtensionsDialog::VioraExtensionsDialog(QWidget* parent)
    : QDialog(parent)
    , m_tabs(nullptr)
...
VioraExtensionsDialog::~VioraExtensionsDialog() {}

void VioraExtensionsDialog::setupUI() {
...
void VioraExtensionsDialog::setupInstalledTab() {
    ...
    connect(m_refreshBtn, &QPushButton::clicked, this, &VioraExtensionsDialog::refreshPluginList);
    connect(m_reloadScriptsBtn, &QPushButton::clicked, this, [this]() {
        ExtensionManager::instance().reloadAll();
        refreshPluginList();
    });
    connect(m_pluginList, &QListWidget::itemClicked, this, &VioraExtensionsDialog::onPluginSelected);
    connect(m_toggleEnabledBtn, &QPushButton::clicked, this, &VioraExtensionsDialog::onToggleSelectedPluginEnabled);
    connect(m_uninstallBtn, &QPushButton::clicked, this, &VioraExtensionsDialog::onUninstallSelectedPlugin);
...
void VioraExtensionsDialog::setupOnlineTab() {
    ...
    connect(m_onlineRefreshBtn, &QPushButton::clicked,
            this, &VioraExtensionsDialog::refreshOnlinePluginList);
    connect(m_onlineSearchEdit, &QLineEdit::returnPressed,
            this, &VioraExtensionsDialog::refreshOnlinePluginList);
    connect(m_onlinePluginList, &QListWidget::itemClicked,
            this, &VioraExtensionsDialog::onOnlinePluginSelected);
    connect(m_onlineDownloadBtn, &QPushButton::clicked,
            this, &VioraExtensionsDialog::onDownloadSelectedPlugin);
    connect(m_onlineRetryBtn, &QPushButton::clicked,
            this, &VioraExtensionsDialog::onRetryDownload);
}

void VioraExtensionsDialog::setupUpdatesTab() {
    ...
    connect(m_updatesRefreshBtn, &QPushButton::clicked,
            this, &VioraExtensionsDialog::refreshUpdatesList);
    connect(m_updateSelectedBtn, &QPushButton::clicked,
            this, &VioraExtensionsDialog::onUpdateSelectedPlugin);
    connect(m_updateAllBtn, &QPushButton::clicked,
            this, &VioraExtensionsDialog::onUpdateAllPlugins);
    connect(m_updatesList, &QListWidget::itemClicked,
            this, &VioraExtensionsDialog::onUpdateItemSelected);
}

void VioraExtensionsDialog::refreshPluginList() {
    m_pluginList->clear();
...
void VioraExtensionsDialog::onPluginSelected(QListWidgetItem* item) {
...
void VioraExtensionsDialog::onToggleSelectedPluginEnabled() {
...
void VioraExtensionsDialog::onUninstallSelectedPlugin() {
    QListWidgetItem* currentItem = m_pluginList->currentItem();
    if (!currentItem) return;
...
void VioraExtensionsDialog::refreshOnlinePluginList() {
...
void VioraExtensionsDialog::onOnlinePluginSelected(QListWidgetItem* item) {
    const QVariant indexData = item->data(Qt::UserRole);
    if (!indexData.isValid()) {
        m_onlineDetailsLabel->setText("Select an online plugin to see details.");
        return;
...
QString VioraExtensionsDialog::detectedPlatformTag() const {
...
QString VioraExtensionsDialog::detectedAppVersion() const {
...
void VioraExtensionsDialog::refreshUpdatesList() {
...
QString VioraExtensionsDialog::formatUpdateDetails(const QJsonObject& update) const {
...
void VioraExtensionsDialog::onUpdateItemSelected(QListWidgetItem* item) {
    const QVariant indexData = item->data(Qt::UserRole);
    if (!indexData.isValid()) {
        m_updatesDetailsLabel->setText("Select an update to see details.");
        m_updateSelectedBtn->setEnabled(false);
...
void VioraExtensionsDialog::onUpdateSelectedPlugin() {
...
void VioraExtensionsDialog::onUpdateAllPlugins() {
...
void VioraExtensionsDialog::processNextQueuedUpdate() {
...
void VioraExtensionsDialog::updateOnlineStatus(const QString& text) {
    if (m_onlineStatusLabel) {
        m_onlineStatusLabel->setText(text);
    }
}

void VioraExtensionsDialog::updateUpdatesStatus(const QString& text) {
    if (m_updatesStatusLabel) {
        m_updatesStatusLabel->setText(text);
    }
}

QString VioraExtensionsDialog::formatOnlinePluginDetails(const QJsonObject& plugin) const {
    const QString name = plugin.value("name").toString(plugin.value("id").toString("(unknown)"));
...
QString VioraExtensionsDialog::compatibilitySummary(const QJsonObject& versionObj) const {
...
QString VioraExtensionsDialog::verificationSummary(const QJsonObject& versionObj) const {
    if (versionObj.isEmpty()) {
...
QString VioraExtensionsDialog::defaultDownloadPath(const QString& pluginId, const QString& version) const {
...
void VioraExtensionsDialog::onDownloadSelectedPlugin() {
...
void VioraExtensionsDialog::onRetryDownload() {
    ...
    connect(m_activeDownloadReply, &QNetworkReply::downloadProgress,
            this, &VioraExtensionsDialog::onDownloadProgress);
    connect(m_activeDownloadReply, &QNetworkReply::finished,
            this, &VioraExtensionsDialog::onDownloadFinished);
    ...
    connect(m_downloadProgressDialog, &QProgressDialog::canceled,
            this, &VioraExtensionsDialog::onDownloadCanceled);
...
bool VioraExtensionsDialog::startDownloadForPluginVersion(const QString& pluginId,
...
void VioraExtensionsDialog::startDownloadFromSelection(const QJsonObject& plugin, bool isRetry) {
...
void VioraExtensionsDialog::onDownloadProgress(qint64 bytesReceived, qint64 bytesTotal) {
...
void VioraExtensionsDialog::onDownloadFinished() {
    if (!m_activeDownloadReply) {
        return;
    }
...
void VioraExtensionsDialog::onDownloadCanceled() {
    if (m_activeDownloadReply) {
...
void VioraExtensionsDialog::resetDownloadState() {
    if (m_downloadProgressDialog) {
...
void VioraExtensionsDialog::forceVTable() {}
#include "viora_extensions_dialog.moc"
