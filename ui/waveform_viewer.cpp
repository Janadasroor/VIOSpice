...
void WaveformViewer::exportSignalsCsv() {
    ...
    QList<CsvSignal> exportList;
    for (const auto& n : names) {
        const auto& sig = m_signals[n];
        CsvSignal csvSig;
        csvSig.name = n;
        csvSig.time = sig.time;
        csvSig.values = sig.values;
        exportList.append(csvSig);
    }

    if (!WaveformCsvHelper::exportCsv(path, exportList)) {
        QMessageBox::warning(this, "Export CSV", "Failed to write CSV file.");
    }
}
...
void WaveformViewer::onExpressionSubmitted(const QString &expression, const QColor &color, const QString &targetName) {
    ...
    if (hasExpression) {
        if (!WaveformExpressionParser::parseExpression(expression, m_signals.keys(), signalNames, error)) {
        ...
        QMap<QString, WaveformExpressionParser::SignalData> parserSignals;
        for (auto it = m_signals.constBegin(); it != m_signals.constEnd(); ++it) {
            WaveformExpressionParser::SignalData psd;
            psd.time = it.value().time;
            psd.values = it.value().values;
            parserSignals.insert(it.key(), psd);
        }

        QVector<double> time, values;
        if (!WaveformExpressionParser::evaluateExpression(expression, signalNames, parserSignals, time, values)) {
...
void WaveformViewer::loadCsv(const QString& filePath) {
    ...
    QList<CsvSignal> importedSignals;
    QString error;
    if (!WaveformCsvHelper::loadCsv(path, importedSignals, error)) {
        QMessageBox::warning(this, "Import CSV", error);
        return;
    }
    ...
    for (const auto& sig : importedSignals) {
        addSignal(sig.name, sig.time, sig.values);
    }

    updatePlot(true);
    QMessageBox::information(this, "Import CSV",
        QString("Imported %1 signal(s) with %2 points from CSV.")
            .arg(importedSignals.size())
            .arg(importedSignals[0].time.size()));
}
...
void WaveformViewer::showAnalysisForSeries(const QString &seriesName) {
    ...
    if (!m_acMode && data.type != SignalType::POWER && data.time.size() >= 3) {
        auto edges = WaveformMathProcessor::computeEdgeTimes(data.time, data.values);
